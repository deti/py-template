...
from {{cookiecutter.py_package}}.cli.serve import main
from {{cookiecutter.py_package}}.settings import Settings, get_settings
...
@patch("{{cookiecutter.py_package}}.cli.serve.uvicorn.run")
def test_serve_main_function(mock_uvicorn_run):
    ...
    with patch(
        "{{cookiecutter.py_package}}.cli.serve.get_settings",
...
def test_serve_module_can_be_imported():
    ...
    import {{cookiecutter.py_package}}.cli.serve  # noqa: PLC0415

    # Verify the module has the expected attributes
    assert hasattr({{cookiecutter.py_package}}.cli.serve, "main")
    assert callable({{cookiecutter.py_package}}.cli.serve.main)
    assert hasattr({{cookiecutter.py_package}}.cli.serve, "app")
...
def test_serve_with_cli_host_parameter(mock_uvicorn_run):
    ...
    with patch("{{cookiecutter.py_package}}.cli.serve.get_settings") as mock_get_settings:
        mock_settings = MagicMock()
        mock_settings.host = "127.0.0.1"
        mock_settings.port = 8001
        mock_settings.log_level = "INFO"
